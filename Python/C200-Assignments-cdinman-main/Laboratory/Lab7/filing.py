...
def readingEx2():
    ...
    This function will be a "workspace" for us to practice reading files
    """
    with open("Laboratory/Lab7/blank.txt","r") as someFiles:
        contents=someFiles.readlines()
        ourPrint(contents)
...
def writeEx1():
    ...
    """
    stuff=['a','b','c','d','e','f']
    with open("Laboratory/Lab7/wrong.txt",'w') as fileToWrite:
        for s in stuff:
            fileToWrite.write(s)
        fileToWrite.close()
...
def writeEx2():
    ...
    This function will be a "workspace" for us to practice reading files
    """
    with open("Laboratory/Lab7/wrong.txt","a") as fileToWrite:
        for s in range(4):
...
def calculation(filePath):
    ...
    split_Lines=[i for i in originalContents.split('\n') if i]
    sum=0
    size=len(split_Lines)
    sum=int(split_Lines[size-2])+int(split_Lines[size-1])
