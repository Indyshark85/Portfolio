...
public class BudgetGUI {
    ...
    class TransactionsPanel extends JPanel {
        ...
        private double readAmount() {
            try {
                ...
                if (v <= 0) throw new NumberFormatException(); // ← must be <= 0, not < 0
                return v;
            } catch (NumberFormatException ex) {
    ...
    class ReportsPanel extends JPanel {
        private final JComboBox<String> accCombo; // ← promote to field
        ReportsPanel() {
            super(new BorderLayout(0, 16));
            setBackground(BG);
            setBorder(new EmptyBorder(24, 28, 24, 28));
            ...
            accCombo = styledCombo(); //added for refreshing bar
        ...
        void refresh() {
            String prev = (String) accCombo.getSelectedItem();
            accCombo.removeAllItems();
            session.getAccounts().forEach(a -> accCombo.addItem(a.getName()));
            if (prev != null) accCombo.setSelectedItem(prev);
        }
    }
