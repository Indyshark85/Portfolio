...
public class PDFStrategy implements ReportFormatter {
    ...
    private String buildContentStream(Account account) {
        ...
        double pct = Math.max(0.0, account.getBudgetLimit() > 0
                ? (account.getBalance() / account.getBudgetLimit()) * 100 : 0); //changed to clamp negative percentages
    ...
    private String escape(String s) {
        //Replaced non-Latin-1 chars with '?' to keep byte count predictable
        String sanitized = s.chars()
                .map(c -> c > 255 ? '?' : c)
