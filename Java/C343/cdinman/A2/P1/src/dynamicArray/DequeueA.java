...
public class DequeueA<E> implements DequeueI<E> {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof DequeueA<?> other)) return false;
        if (this.size != other.size) return false;

        for (int x=0; x<size; x++){
            int index0=Math.floorMod(back+1+x,capacity);
            int index1=Math.floorMod(other.back+1+x,other.capacity);
            try {
                if (!elems[index0].getValue().equals(other.elems[index1].getValue())) {
                    return false;
                }
            } catch (EmptyDequeueE e) {
                return false; // shouldn’t happen if sizes match
            }
    ...
    public int hashCode() {
        ...
        for (int x=0;x<size;x++){
            int index0=Math.floorMod(back+1+x,capacity);
            try {
                result = 31 * result + elems[index0].getValue().hashCode();
            } catch (EmptyDequeueE e) {
                // skip empties
            }
        }
