...
public class Extendible<K, V> extends AbstractHM<K, V> {
    ...
    public void rehash() {
        if (globalDepth >= 30) {
            throw new IllegalStateException("Global depth limit reached");
        }
        ...
        // Duplicate each prior slot across the new upper half
        for (int i = 0; i < newSize; i++) {
            newDir.add(directory.get(i % oldSize));
        }
        directory = newDir;
    ...
    private void splitBucket(int idx) {
        ...
        // 1) Expand directory if needed
        if (bucket.localDepth == globalDepth) {
            rehash();
            bucket = directory.get(idx);
        }

        // Capture old local depth before increment
        int oldDepth = bucket.localDepth;

        // 2) Bump local depth and create a new bucket at that depth
        bucket.localDepth = oldDepth + 1;
        Bucket newBucket = new Bucket(bucket.localDepth);

        // 3) Reassign directory pointers for all aliases that belonged to this bucket's old prefix
        int dirSize = 1 << globalDepth;
        int oldMask = (1 << oldDepth) - 1;       // match family by old depth
        int newBitMask = 1 << oldDepth;          // the new highest bit to branch on
        int familyPrefix = idx & oldMask;

        for (int i = 0; i < dirSize; i++) {
            if ((i & oldMask) == familyPrefix) {
                // decide by the new highest bit
                if ((i & newBitMask) != 0) {
        ...
        // 4) Redistribute entries using GLOBAL index
        List<Entry<K, V>> oldEntries = new ArrayList<>(bucket.entries);
        bucket.entries.clear();
        newBucket.entries.clear();

        for (Entry<K, V> e : oldEntries) {
            int newIndex = hash(e.getKey()); // uses globalDepth
