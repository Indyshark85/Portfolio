...
public abstract class Probing<K,V> extends AbstractHM<K,V> {
    ...
    public @NotNull Set<K> keySet() {
        ...
        for (AbstractEntry<K,V> entry : table) {
            if (entry instanceof Entry<K,V> e) {
                keys.add(e.getKey());
            }
        }
        return keys;
    ...
    public boolean containsKey(@NotNull K key) {
        for (int i = 0; i < capacity; i++) {
            int j = hash(key, i);
            if (table[j] instanceof EmptyP) return false;
            if (table[j] instanceof Entry<K,V> entry && entry.getKey().equals(key)) {
                return true;
            }
            if (table[j] instanceof Marked) continue; // skip tombstones
        }
        return false;
    }
    ...
    public boolean containsValue(@NotNull V value) {
        for (AbstractEntry<K,V> entry : table) {
            if (entry instanceof Entry<K,V> e && e.getValue().equals(value)) {
                return true;
            }
        }
        return false;
    ...
    public void rehash() {
        ...
        AbstractEntry<K,V>[] oldTable = table;
        table = (AbstractEntry<K, V>[]) Array.newInstance(AbstractEntry.class, newCapacity);
        capacity = newCapacity;
        size = 0;
        for (int i = 0; i < capacity; i++) table[i] = new EmptyP<>();

        for (AbstractEntry<K,V> entry : oldTable) {
            if (entry instanceof Entry<K,V> e) {
    ...
    public void put(@NotNull K key, @NotNull V value) {
        ...
        for (int i = 0; i < capacity; i++) {
            ...
            else if (table[j] instanceof EmptyP || table[j] instanceof Marked) {
                // Empty or tombstone → reuse slot
                table[j] = new Entry<>(key, value);
                size++;
                return;
            }
        }
        // If we probed the whole table and didn’t find a spot, rehash and retry
        rehash();
        put(key, value);
    ...
    public @NotNull V get(@NotNull K key) throws KeyNotFoundE {
        for (int i = 0; i < capacity; i++) {
            int j = hash(key, i);
            if (table[j] instanceof EmptyP) break;
            if (table[j] instanceof Entry<K,V> entry && entry.getKey().equals(key)) {
                return entry.getValue();
            }
            if (table[j] instanceof Marked) continue; // keep probing
        }
