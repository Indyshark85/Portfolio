...
public class Chained<K,V> extends AbstractHM<K,V> {
    ...
    public void rehash() {
        ...
        for (int i = 0; i < newCapacity; i++) newTable[i] = new LinkedList<>();

        for (LinkedList<Entry<K,V>> bucket : table) {
            for (Entry<K,V> entry : bucket) {
                int newIndex = Math.floorMod(hashFun.apply(entry.getKey()), newCapacity);
                newTable[newIndex].add(new Entry<>(entry.getKey(), entry.getValue()));
            }
        }
