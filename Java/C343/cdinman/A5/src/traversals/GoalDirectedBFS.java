...
public class GoalDirectedBFS<V> implements TraversalStrategy<V> {
    ...
    public boolean hasNext() {
        return !toVisit.isEmpty() && path.isEmpty();
    }
    ...
    public Progress visit(V current, Collection<V> neighbors) {
        // Do not poll here; next() already removed the node.
        ...
        neighborList.sort(Comparator.comparing(Object::toString));

        for (V n : neighborList) {
